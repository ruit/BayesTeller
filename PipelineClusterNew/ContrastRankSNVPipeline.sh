...
awk -v N=$total -v CUT=$cutoff '{if ($1 == $3 && ($2/N) <= CUT) print $1"\t"$4"\t"N"\t"($2/N)}' >$cancer".bloc"
		...
		#generate model file for BayesTeller @Dec1, 2014
		cat $cancer".bloc.Pval" | awk '{if ($4 == 0) {$4=$4+(0.5/$3)};print $1"\t"($2/$3)"\t"$4"\t"$5}' | sort -k4 -n -r | head -n1000 >$cancer".mod"
