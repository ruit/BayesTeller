...
CrossVD(){
	...
	cat $tempdir"/modelA" | sort -k3 -n -r > $tempdir"/modelA.sorted"
	rm $tempdir"/modelA"

	cat $tempdir"/modelB" | sort -k3 -n -r > $tempdir"/modelB.sorted"
	rm $tempdir"/modelB"

	cat $tempdir"/testPatList4A.tab" | while IFS=\t read testPatient
		do
 			#echo $testPatient
			cat $tumorSample | grep -w $testPatient | cut -f1 >$tempdir"/"$tumorSample"_"$testPatient".snvsa"
			cat $germlineSample | grep -w $testPatient | cut -f1 >$tempdir"/"$germlineSample"_"$testPatient".snvsa"
	...
	cat $tempdir"/testPatList4B.tab" | while IFS=\t read testPatient
		do
 			#echo $testPatient
			cat $tumorSample | grep -w $testPatient | cut -f1 >$tempdir"/"$tumorSample"_"$testPatient".snvsb"
			cat $germlineSample | grep -w $testPatient | cut -f1 >$tempdir"/"$germlineSample"_"$testPatient".snvsb"
